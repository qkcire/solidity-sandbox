...
contract SimpleAuction {
  ...
  function auctionEnd() external {
    ...
    // interact with other contracts (i.e. they call
    // functions or send Ether) into three phases:
    // 1. checking conditions
    // 2. performing actions (potentially changing conditions)
    // 3. interacting with other contracts
    // If these phases are mized up, the other contract could
    // call back into the current contract and modify the state
    // or cause effects (ether payout) to be performed multiple
    // times.
    // If functions called internally include interaction with
    // external contracts, they also have to be considered
